...
(function(a) {
    a.fn.customSelectBox = function(b) {
        b = a.extend({selectboxClass: "styled-select", buttonClass: "curr", selectedClass: "selected", disabledClass: "disabled", focusedClass: "focused"}, b);
        return this.each(function() {
            var g = a(this), c = 0;
            g.css("visibility", "hidden").wrap('<div class="' + b.selectboxClass + '"></div>');
            var d = g.parent(), f = d.find("option"), e = g.find("option:selected").html();
            d.append('<span class="' + b.buttonClass + '">' + e + '</span><ul style="display:none;"></ul>');
            f.each(function(l) {
                var k = a(this), m = k.val() ? k.val() : k.html(), h = k.html(), j = k.is(":selected") ? b.selectedClass : k.is(":disabled") ? b.disabledClass : "";
                var fi = k.attr('data-filter');
                if (fi !== undefined) {
                    d.find("ul").append('<li class="' + j + " o-" + l + ' filter" data-value="' + m + '"' + ' data-filter="' + fi + '">' + h + "</li>");
                } else {
