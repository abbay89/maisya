...
class grocery_CRUD_Model extends CI_Model {
    ...
    function get_edit_values($primary_key_value) {
        ...
        if (is_array($primary_key_value)) {
            ...
            foreach ($primary_key_value as $value) {
                $this->db->where($primary_key_field[$i], $value);
                $i++;
            }
        } else {
    ...
    function db_update($post_array, $primary_key_value) {
        ...
        if (is_array($primary_key_value)) {
            ...
            $i = 0;
            foreach ($primary_key_value as $value) {
                $primary_key[$primary_key_field[$i]] = $value;
                $i++;
            }
        } else {
            $primary_key = array($primary_key_field => $primary_key_value);
        }
        $this->db->update($this->table_name, $post_array, $primary_key);
        $act['type'] = "UPDATE";
        $act['new_value'] = implode("|", $post_array);
        $this->create_activity_log($act);
    }
    ...
    function db_delete($primary_key_value) {
        ...
        $this->db->limit(1);
        $this->db->delete($this->table_name, $primary_key);

        $act['type'] = "DELETE";
        $act['new_value'] = implode("|", $primary_key);
        $this->create_activity_log($act);
    ...
    public function create_activity_log($data) {
//        $data['user_id'] = $this->session->userdata('who');
