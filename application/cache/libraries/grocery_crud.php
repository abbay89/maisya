...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver {
    ...
    protected function showList($ajax = false, $state_info = null) {
        ...
        $data->export_url = $this->getExportToExcelUrl($this->export_url);
        ...
        foreach ($data->list as $num_row => $row) {
            $data->list[$num_row]->edit_url = $data->edit_url . '/' . $this->primary_to_string($data->primary_key, $row);
            $data->list[$num_row]->delete_url = $data->delete_url . '/' . $this->primary_to_string($data->primary_key, $row);
        }
    ...
    protected function primary_to_string($primary_key, $row = null) {
        if (is_array($primary_key)) {
            $temp = '';
            foreach ($primary_key as $key) {
                $temp .= isset($row) ? ($row->{$key} . '/') : ($key . '/');
            }
            return $temp;
        } else {
            return isset($row) ? $row->{$primary_key} : $primary_key;
        }
    }
    ...
    protected function insert_layout($insert_result = false) {
        ...
        if ($insert_result === false) {
            ...
            if (!$this->unset_back_to_list) {
                $success_message .= " <a href='#' onclick ='close_add()'>" . $this->l('form_go_back_to_list') . "</a>";
            }
    ...
    protected function get_layout() {
        ...
        if ($this->echo_and_die === false) {
            ...
            return (object) array('output' => $this->views_as_string, 'js_files' => $js_files, 'css_files' => $css_files, 'title' => $this->subject);
    ...
    protected function get_time_input($field_info, $value) {
        ...
        if (!empty($value) && $value != '0000-00-00 00:00:00' && $value != '00:00:00') {
            list($year, $month, $day) = explode('-', substr($value, 0, 10));
            $date = date($this->php_date_format, mktime(0, 0, 0, $month, $day, $year));
            $datetime = $date . substr($value, 10);
        } else {
...
class grocery_CRUD_States extends grocery_CRUD_Layout {
    ...
    protected function getExportToExcelUrl($url) {
        if ($url == 'export') {
            return $this->state_url($url);
        } else {
            $temp = explode("/", $this->state_url($url));
            $new_url = "";
    ...
    protected function getValidationUpdateUrl($primary_key = null) {
        if ($primary_key === null)
            ...
            return $this->state_url('update_validation/' . $this->primary_to_string($primary_key));
    }
    ...
    protected function getUpdateUrl($state_info) {
        return $this->state_url('update/' . $this->primary_to_string($state_info->primary_key));
    }
    ...
    protected function getDeleteUrl($state_info = null) {
        if (empty($state_info))
            ...
            return $this->state_url('delete/' . $this->primary_to_string($state_info->primary_key));
    }
        ...
        switch ($state_code) {
            ...
            case 3:
                if ($first_parameter !== null) {
                    if (isset($second_parameter)) {
                        $state_info = (object) array('primary_key' => array($first_parameter, $second_parameter));
                    } else {
            ...
            case 6:
                if (!empty($_POST) && $first_parameter !== null) {
                    if (isset($second_parameter)) {
                        $state_info = (object) array('primary_key' => array($first_parameter, $second_parameter), 'unwrapped_data' => $_POST);
                    } else {
...
class grocery_CRUD extends grocery_CRUD_States {
    ...
    public function set_export_url($url) {
        $this->export_url = $url;
        return $this;
    }
