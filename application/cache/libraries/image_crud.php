...
class image_CRUD {
    ...
    function set_ready_field($field) {
        $this->ready_field = $field;
    ...
    protected function _change_ready($primary_key, $value) {
        $this->ci->db->update($this->table_name, array($this->ready_field => $value), array($this->primary_key => $primary_key));
    }
    ...
    protected function _insert_table($file_name, $relation_id = null) {
        ...
        if ($this->callback_before_insert !== null) {
            $callback_return = call_user_func($this->callback_before_insert, $insert);

            if (!empty($callback_return) && is_array($callback_return))
                $insert = $callback_return;
    ...
    public function getState() {
        ...
        if (isset($rsegments_array[3]) && is_numeric($rsegments_array[3])) {
            ...
            $change_ready_url = site_url('admin/'.$rsegments_array[1] . '/' . $rsegments_array[2] . '/change_ready');
            ...
            $state['change_ready_url'] = $change_ready_url;
            ...
            return (object) $state;
        } elseif (isset($rsegments_array[3]) && $rsegments_array[3] == 'change_ready') {
            $state = array('name' => 'change_ready');
            return (object) $state;
        }
    }
            ...
            switch ($state_info->name) {
                case 'list':
                    ...
                    $this->_library_view('list.php', array(
                        ...
                        'change_ready_url' => $state_info->change_ready_url,
                        'photos' => $photos,
                ...
                case 'change_ready':
                    @ob_end_clean();
                    $this->_change_ready($_POST['primary_key'], $_POST['value']);
                    die();
                    break;
            }
        }
