...
class Reff extends CI_Controller {
	...
	public function metatag() {
        try {
            $crud = new grocery_CRUD();
            $crud->set_table('meta_tag');
            $crud->set_subject('Detail Meta Tag');
	...
	public function maks_book() {
        try {
            $crud = new grocery_CRUD();
            $crud->set_table('tb_infbook');
            $crud->set_subject('Maksimal Book');
            $crud->display_as('inf_desc', 'Message');
            $crud->display_as('inf_time', 'Time End on Hour');
            $crud->columns('inf_desc', 'inf_time');
            $crud->unset_fields('inf_book');
            $this->load($crud);
        } catch (Exception $e) {
            show_error($e->getMessage() . ' --- ' . $e->getTraceAsString());
        }
	...
	public function paymentguide() {
        try {
            $crud = new grocery_CRUD();
            $crud->set_table('tb_guide');
            $crud->set_subject('Guide Payment');
            $crud->display_as('guide_title', 'Guide Title');
            $crud->display_as('guide_content', 'Guide Desc');
            $crud->display_as('guide_pic', 'Guide Img');
            $crud->display_as('guide_loc', 'Img Location');
            $crud->set_field_upload('guide_pic', 'assets/uploads/reff');
            $this->load($crud);
        } catch (Exception $e) {
            show_error($e->getMessage() . ' --- ' . $e->getTraceAsString());
        }
	...
	public function img_setting() {
        try {
            $crud = new grocery_CRUD();
            $crud->set_table('img_content');
            $crud->set_subject('Image Setting');
            $crud->display_as('imgfile', 'Img Location');
            $crud->set_field_upload('imgfile', 'assets/uploads/reff');
            $this->load($crud);
        } catch (Exception $e) {
            show_error($e->getMessage() . ' --- ' . $e->getTraceAsString());
        }
