...
$this->set_css('assets/image_crud/css/checkbox-custom.css');
...
<?php } ?>
    ...
    function change_ready(data_id) {
        $.ajax({
            url: '<?php echo $change_ready_url; ?>',
            type: 'post',
            data: {primary_key: data_id, value: $("#check" + data_id + "").is(":checked") ? 1 : 0},
            beforeSend: function()
            {
                $('.file-upload-messages-container:first').show();
                $('.file-upload-message').html("Save Ready Image");
            },
            complete: function()
            {
                $('.file-upload-messages-container').hide();
        ...
        <ul class='photos-crud'>
            <?php foreach ($photos as $photo_num => $photo) {
                ...
                <li id="photos_<?php echo $photo->$primary_key; ?>">
                    <div class='photo-box'>
                        <a href='<?php echo $photo->image_url ?>' <?php
                        if (isset($photo->title)) {
                            echo 'title="' . str_replace('"', "&quot;", $photo->title) . '" ';
                        }
                        ?>target='_blank' class="color-box" rel="color-box" tabindex="-1"><img src='<?php echo $photo->thumbnail_url ?>' width='96' height='96' class='basic-image' /></a>
                        ...
                        <?php if ($ready_field) { ?>
                            <div class="checkbox">
                                <input id="check<?php echo $photo->$primary_key ?>" <?php echo $photo->$ready_field ? 'checked' : '' ?> type="checkbox" onchange="return change_ready('<?php echo $photo->$primary_key ?>')"value="1" />
                                <label for="check<?php echo $photo->$primary_key ?>"><span>Check-me</span></label>
                            </div>
                        <?php } ?>
                        <div class="clear"></div>
                    </div>
