...
class Image_moo {
    ...
    public function resize_crop_png($mw, $mh) {
        if (!$this->_check_image())
            return $this;
...
//        $col = $this->_html2rgb("#F");
        $bg = imagecolorallocatealpha($this->temp_image, 0,0,0,126);
...
//		    if( $this->width > $this->height ) { could calc wronf - Cole Thorsen swapped to his suggestion
    ...
    public function watermark($position, $offset = 8, $abs = FALSE) {
        ...
        if ($this->watermark_method == 1) {
            ...
            // DFuse normal with selected transparency colour
