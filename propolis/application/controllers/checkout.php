...
class Checkout extends CI_Controller {
	...
	public function save_address(){
		...
		$url='http://www.maisya.id:5060/api/AlamatKirim';
	...
	public function del_address($id){
		$url='http://www.maisya.id:5060/api/AlamatKirim/'.$id;
	...
	public function sendSomaisya($orderid)
	{
		...
		$url='http://www.maisya.id:5060/api/SO';
		$ch = curl_init($url);
	...
	public function minuschart($rowid,$qty){
		if($qty == 1)
		{
			...
			" Delete from shopping_cart where  user_id = '".$this->session->userdata('email')."' and id = '".$rowid."'");		
		}
		else
		{
			$qtyData	= $this->db->query(
			" Update shopping_cart set qty = qty - 1 where  user_id = '".$this->session->userdata('email')."' and id = '".$rowid."'");		
		}
		if ($qtyData) {
		 echo "Success";         
		}else{
	...
	public function pluschart($rowid,$qty){
		//getLastData		
		//$qtyData	= $this->db->query("select qty from shopping_cart where user_id = '".$this->session->userdata('cust_username')."' and id = '".$rowid."'")->row();
		$qtyData	= $this->db->query(
			" Update shopping_cart set qty = qty + 1 where  user_id = '".$this->session->userdata('email')."' and id = '".$rowid."'");		
